...
def click_ok_btn():
    if clickBtn(ok_btn_img, name='okBtn', timeout=2):
        login()
        pass
        # time.sleep(15)
...
def login():
    ...
    if clickBtn(connect_wallet_btn_img, name='connectWalletBtn', timeout=13):
...
def main():
    ...
    while True:
        ...
        if now - last["ok"] > time_intervals_config['check_for_ok_unknown']:
            last["ok"] = now
            sys.stdout.write('\nClicking OK button.')
            click_ok_btn()
            sys.stdout.write("\n")
